...
const navigate = useNavigate();
  ...
  return (
    <div>
      ...
      <Col xs={12} md={6} className="order-md-2 position-relative">
        ...
        <div className='border-0 h-100 d-flex justify-content-center align-items-center p-3 p-md-0' style={{background:'#E27A0C1F'}} >
    ...
    <i className="bi bi-chevron-right" style={{fontSize:'0.8rem'}}></i>
        <NavLink activeclassname='active' className='nav-link'  to="/about-us">About us</NavLink>
          ...
          <div className="d-block gap-3 mt-1">
            <h2 style={{fontWeight:'700', lineHeight:'50px'}}>{heading}</h2>
            <h6>{text}</h6>
          </div>
          <div>
  ...
  const cardStyle = index === 0 
    ? { boxShadow: '0 2px 100px 0 rgba(156, 156, 156, 0.25)', backgroundColor: backgroundColor, border: 'none' }
    : { backgroundColor: backgroundColor, border: 'none' };

  return (
    <Col xs={12} md={4} key={work.id} className="mb-3">
      <Card style={cardStyle} className="d-flex justify-content-center align-items-center p-5">
        <div className="d-flex flex-column gap-3">
          <div style={{ backgroundColor: backgroundTextColor, color: textColor, borderRadius: '8px', width: '100px' }} className="d-flex justify-content-center">
            <p style={{ color: textColor }} className="my-auto">{work.title}</p>
          </div>
      ...
      <section style={{backgroundColor:'#F5F5F5'}} className="mb-5 p-md-5">
        <Row className="px-lg-5">
          ...
          <Col xs={12} md={7} className="mb-3">
            <div className="d-flex flex-column gap-2 ps-md-4 " style={{maxWidth:'34rem'}}>
              <h6 style={{fontWeight:'500', color:'#81AC29'}}>About us</h6>
              
              <h6 style={{lineHeight:'22px'}}>Dehydrator Hub is a vital solution addressing post-harvest loss in Nigeria's agriculture space by connecting producers with processors and processing machinery. By facilitating this matchmaking process, the Dehydrator Hub helps farmers, agro-processors, and exporters access the necessary infrastructure to add value to their produce efficiently. This not only reduces post-harvest losses but also enables farmers to capitalize on their harvest by converting raw produce into processed goods. 
              <br/>  <br/> Additionally, by providing a marketplace for both processing machines and processed food, the Dehydrator Hub further supports the agricultural ecosystem. It allows for the exchange of equipment and products, fostering collaboration and efficiency within the industry. <br/>  <br/> Overall, the Dehydrator Hub plays a crucial role in optimizing the agricultural value chain, reducing waste, and increasing the profitability of farmers and processors alike.</h6>
      ...
      <section className="p-3 p-md-5 mx-lg-5 bg-white">
        <h4 style={{fontWeight:'700'}} className="text-center">Our Team</h4>
        <Row className="my-5 mx-xl-5">
         ...
         <Card className="border-none p-3 justify-content-center text-center align-items-center" style={{background:'#F5F5F5',  border:'none'}}>
          ...
          <Col xs={6} md={4} className="mb-3">
         <Card className="border-none p-3 align-items-center justify-content-center text-center" style={{background:'#F5F5F5',  border:'none'}}>
      ...
      <Col xs={12} md={4} className="mb-3 px-2 px-md-0">
          ...
          <div className="position-relative px-3 text-center align-items-center justify-content-center" style={{ zIndex: 0 }}>
        ...
        <Col xs={12} md={8} className="mb-3 mt-3 mt-md-0">
          <div className="d-flex flex-column gap-3 gap-md-5 ps-3 pe-4 px-md-5 " style={{maxWidth:'34rem'}}>
            <h1>Testimonials</h1>
            <h4 style={{color:'#637381', textDecoration:'italic', fontWeight:'400'}}><i>{quote}</i></h4>
            <div>
              <h4 style={{fontWeight:'600', color:'#212B36'}}>{name}</h4>
              <h6 style={{color:'#637381'}}>{title}</h6>
            </div>
            <div className="d-flex gap-3">
              <button className="btn btn-outline-secondary" onClick={goToPrevious} style={{borderRadius:'50%', border:'none', boxShadow: '0px 4px 4px 0px #00000040'
}}><i className="bi bi-arrow-left"></i></button>
              <button className="btn btn-outline-secondary" onClick={goToNext} style={{borderRadius:'50%', border:'none',boxShadow: '0px 4px 4px 0px #00000040'
